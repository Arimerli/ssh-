...
function DettaglioComponente({ utente }) {
  ...
  async function handleElimina() {
    if (!window.confirm(`Sei sicuro di voler eliminare "${componente.nome}"?`))
      return;
  ...
  function breadcrumb(lista, itemId) {
    ...
    while (corrente) {
      ...
      const parentId = corrente.parent; // ← capture it first
      corrente = lista.find((x) => x.id === parentId); // ← no closure over mutable var
    }
    return percorso;
  }
      ...
      <div className={styles.header}>
        ...
        {(utente?.ruolo === "Amministratore" ||
          ...
          <div className={styles.headerAzioni}>
            ...
            <button className={styles.btnElimina} onClick={handleElimina}>
              Elimina
            </button>
          </div>
        )}
        ...
        <div className={styles.breadcrumb}>
          {breadcrumbCategoria.map((cat, i) => (
            <span key={cat.id}>
              ...
              <span
                className={
                  i === breadcrumbCategoria.length - 1
                    ? styles.breadcrumbAttivo
      ...
      {giacenze.length > 0 && (
        <div className={styles.sezione}>
          <div className={styles.sezioneLabelRow}>
            ...
            <span className={styles.totaleLabel}>
              Totale: <strong>{totale}</strong> pezzi
            </span>
          </div>
          <div className={styles.tabellaWrapper}>
              ...
              <tbody>
                {giacenze.map((g, i) => {
                  ...
                  return (
                    <tr
                      key={g.id}
                      className={i % 2 === 0 ? styles.trPari : styles.trDispari}
                    >
                      ...
                      <td className={styles.td}>
                        <span
                          className={`${styles.indicatore} ${coloreQuantita(g)}`}
                        />
                        {g.quantita}
                      </td>
                      ...
                      <td className={styles.td}>
                        <span
                          className={
                            g.scorta ? styles.scortaSi : styles.scortaNo
                          }
                        >
                          {g.scorta ? "✓ Sì" : "✗ No"}
                        </span>
