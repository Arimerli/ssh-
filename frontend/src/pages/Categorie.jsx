...
function Categorie({ utente }) {
    ...
    console.log("PUO MODIFICARE:", puoModificare);
    ...
    function getTutteLeCategorie(id) {
        ...
        figli.forEach(f => res.push(...getTutteLeCategorie(f.id)));
        return res;
    }

    function contaComponenti(id) {
        const lista = getTutteLeCategorie(id);
        return componenti.filter(c => lista.includes(c.categoria)).length;
    }
    ...
    async function handleModifica(e, id) {
        ...
        await modificaCategoria(id, nuovoNome);
        setModificaId(null);
        setNuovoNome("");
        getCategorie().then(res => setCategorie(res.data));
    }
    ...
    function getIcona(livello) {
        if (livello === 0) return <LuFlaskConical size={16} />;
        ...
        return (
            <div className={styles.card}>
                <div
                    ...
                    onClick={() =>
                        !inModifica && navigate(`/componenti?categoria=${id}`)
                    }
                >
                    <div className={styles.info}>
                        ...
                        <div>
                            <div className={styles.sotto}>
                                {getLivelloLabel(livello)}
                            </div>
                    ...
                    <div className={styles.right}>
                        {puoModificare && inModifica ? (
                            ...
                            <>
                                ...
                                {puoModificare && puoEliminare(id) && (
                                    <span
                                        className={styles.btnElimina}
                                        onClick={e =>
                                            handleElimina(e, id, cat.nome)
                                        }
                                    >
                                        <LuTrash2 size={14} />
                                    </span>
                ...
                {aperto && haFigli && (
                    <div className={styles.sottoLista}>
                        {figli.map(f => (
                            <Nodo key={f.id} id={f.id} livello={livello + 1} />
                        ))}
                    </div>
                )}
            </div>
    ...
    return (
        <div className={styles.container}>
            <div className={styles.lista}>
                {categorieMadre.map(c => (
                    <Nodo key={c.id} id={c.id} livello={0} />
                ))}
            </div>
        </div>
    );
